...
import (
	...
	"github.com/EchoUtopia/pg2oracle/pkg/postgres/util/syncutil"
	"github.com/EchoUtopia/pg2oracle/pkg/postgres/util/timeutil"
)
...
func (c *Clock) getPhysicalClockLocked() int64 {
	...
	if c.mu.lastPhysicalTime != 0 {
		...
		if interval > int64(c.maxOffset/10) {
			...
			log.Printf("backward time jump detected (%f seconds)\n", float64(newTime-c.mu.lastPhysicalTime)/1e9)
		}
	}
...
func (c *Clock) Update(rt Timestamp) Timestamp {
	...
	if rt.WallTime > c.mu.timestamp.WallTime {
		...
		if c.maxOffset > 0 && offset > c.maxOffset {
			log.Printf("remote wall time is too far ahead (%s) to be trustworthy - updating anyway\n", offset)
		}
