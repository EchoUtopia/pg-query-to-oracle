...
func recurse(cur interface{}) (src string) {
	...
	// DNull = dNull{} in the parser code
	if curTyp.String() == "parser.dNull" {
		src = ptr + "parser.DNull"
		return src

	} else if curTyp.Kind() == reflect.Struct && curTyp.String() == "parser.StrVal" {
		src = `parser.NewDString`

	} else if curTyp.Kind() == reflect.Bool {
		if curVal.Bool() {
			src = "parser.DBoolTrue"
		} else {
			src = "parser.DBoolFalse"
		}
		return
